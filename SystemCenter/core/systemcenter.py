...
from SystemCenter.model.schema import (SystemParameters, FileInfo, FileUploadRequest, CenterContext,
...
from SystemCenter.routes.routes import SystemCenterRoutes
...
class SystemCenter:
    ...
    def generate_unique_id(self, kind: str, data: Optional[Dict] = None) -> Optional[str]:
        ...
        if kind == 'file':
            return self.cryptoservice.digest_message(f"{data['file_path']}||{data['file_hash']}".encode())
