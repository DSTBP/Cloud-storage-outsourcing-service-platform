...
from utils.builtin_tools.ellipticCurve import Curve, Point, Util
from utils.builtin_tools.encryption import Hash, AES, ECC, SM2, Base64, FASTAES
...
class CryptoService:
    ...
    @classmethod
    def ecc_private_key_pem_to_int(cls, pem_data: str) -> int:
        """将用户上传的 pem 格式私钥转成整型形式"""
        try:
            ...
            if not isinstance(key, ec.EllipticCurvePrivateKey):
                raise ValueError("提供的密钥不是 ECC 私钥")

            private_numbers = key.private_numbers()
            return private_numbers.private_value
        except Exception as e:
            logger.error(f"私钥转换失败: {str(e)}")
            raise
